...
@section('page-subtitle', 'Select tests and assign them to a patient')

@section('topbar-actions')
<a href="{{ route('tenant.orders.index', $currentTenant->slug) }}" class="btn-secondary text-sm">&larr; Back</a>
@endsection
...
<div class="max-w-3xl" x-data="{
    ...
    <form method="POST" action="{{ route('tenant.orders.store', $currentTenant->slug) }}" class="space-y-6">
        @csrf
        ...
        <div class="glass-card p-6 space-y-5">
            ...
            <div class="flex items-center gap-3 p-3 rounded-xl bg-indigo-500/10 border border-indigo-500/20">
                ...
                <p class="text-indigo-300 text-sm">Linked to appointment: <strong>{{ $appointment->scheduled_at->format('d M Y, H:i') }}</strong></p>
            </div>
            @endif
                    ...
                    <div x-show="patientOpen"
                        ...
                        <template x-if="filteredPatients.length === 0">
                            <div class="px-4 py-3 text-sm" style="color:#94a3b8;">
                                No patients found
                                <a href="{{ route('tenant.patients.create', $currentTenant->slug) }}"
                                   class="ml-2 text-indigo-500 underline">Register patient</a>
                            </div>
                        </template>
        ...
        <div class="glass-card p-6">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-white font-semibold">Select Tests</h3>
                <div class="text-right">
                    <p class="text-white/40 text-xs"><span x-text="selectedTests.length"></span> selected</p>
                    <p class="text-white font-semibold">Total: <span x-text="'PKR ' + Math.round(total).toLocaleString()"></span></p>
                </div>
            </div>
            ...
            {{-- Grouped test list (rendered via Alpine x-for so search works) --}}
            <div class="space-y-6">
                <template x-if="groupedTests.length === 0">
                    <p class="text-white/30 text-sm text-center py-6">No tests match "<span x-text="testSearch"></span>"</p>
                </template>

                <template x-for="group in groupedTests" :key="group.cat">
                    <div>
                        <p class="text-white/40 text-xs uppercase tracking-wider mb-2" x-text="group.cat"></p>
                        <div class="space-y-2">
                            <template x-for="test in group.tests" :key="test.id">
                                <label class="flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all"
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <span class="text-white text-sm font-medium" x-text="test.name"></span>
                                        <template x-if="test.is_panel">
                                            <span class="ml-2 text-xs px-2 py-0.5 rounded-full" style="background:rgba(139,92,246,0.2); color:#a78bfa;">Panel</span>
                                        </template>
                                        <template x-if="test.code">
                                            <span class="text-white/30 text-xs ml-1" x-text="test.code"></span>
                                        </template>
                                    </div>
                                    <span class="text-white/60 text-sm font-medium flex-shrink-0" x-text="'PKR ' + Math.round(parseFloat(test.price)).toLocaleString()"></span>
                                </label>
                            </template>
                        </div>
                    </div>
                ...
                @if($tests->isEmpty())
                <p class="text-white/30 text-sm text-center py-6">No active tests in catalog. <a href="{{ route('tenant.tests.create', $currentTenant->slug) }}" class="text-indigo-400 hover:underline">Add tests first</a>.</p>
                @endif
            </div>
        </div>

        <div x-show="selectedTests.length > 0" class="glass-card p-4 border-indigo-500/20 bg-indigo-500/5">
            <p class="text-white/60 text-sm"><span class="text-white font-semibold" x-text="selectedTests.length"></span> test(s) selected — an invoice for <strong x-text="'PKR ' + Math.round(total).toLocaleString()"></strong> will be auto-generated.</p>
        </div>

        <div class="flex gap-3">
            <button type="submit" class="btn-primary" :disabled="selectedTests.length === 0">Create Order</button>
            <a href="{{ route('tenant.orders.index', $currentTenant->slug) }}" class="btn-secondary">Cancel</a>
        </div>
    </form>
</div>
@endsection
