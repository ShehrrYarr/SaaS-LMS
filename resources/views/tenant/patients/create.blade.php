...
<a href="{{ route('tenant.patients.index', $currentTenant->slug) }}" class="btn-secondary text-sm">&larr; Back</a>
@endsection

@section('content')
<div class="max-w-2xl">
    <form method="POST" action="{{ route('tenant.patients.store', $currentTenant->slug) }}" class="space-y-6">
        @csrf
        <div class="glass-card p-8 space-y-5">
            ...
            <div class="border-t border-white/10 pt-5">
                <label class="flex items-start gap-3 cursor-pointer">
                    ...
                    <div>
                        <p class="text-white text-sm font-medium">Send Portal Credentials</p>
                        <p class="text-white/40 text-xs mt-0.5">Patient will receive a welcome email with their login credentials using this lab's SMTP settings. Requires SMTP to be configured in Settings.</p>
                    </div>
                </label>
            </div>
        </div>
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('tenant.patients.index', $currentTenant->slug) }}" class="btn-secondary">Cancel</a>
        </div>
    </form>
</div>
@endsection
